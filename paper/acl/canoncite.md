...
- **C3 — Method.** A joint discriminative exact-ID selector (E2) that attains the lowest misattribution rate of any system at two LLM calls per item against the SOTA's 10–17 (1.9× fewer prompt tokens), by eliminating *implausible* misattributions and concentrating the residual error on genuine near neighbours—above a reader capacity threshold between 8B and 14B, below which the decomposed baseline is the more robust choice. Both the advantage and the reversal survive an end-to-end replication (§Systems and Results).
...
***The gate.** Cross-lingual misattribution rate↓ per corpus, **all ten corpora**, Qwen2.5-14B reader. E2 attains the lowest overall MAR, at 2 LLM calls/item versus D's 10–17. Per-corpus differences smaller than the measured per-cell σ=0.030 should not be read as orderings. The three losses are the low-resource-script corpora where every system degrades badly (MAR >0.8 for Gurmukhi and Rāmāyaṇa). Cross-lingual F1: D 0.440, E2 0.423—F1 is not what a verification layer optimises.*
...
MAR's denominator invites a specific objection: a system that declines the hard items can post a low misattribution rate without discriminating better, and E2 does have the highest wrong-abstention rate of the three (0.487 versus D's 0.439). We therefore recompute the comparison with abstention removed from the denominator entirely—wrong citations per *item attempted*, not per citing item. Cross-lingually, E2 makes 0.137 wrong citations per item against D's 0.153 and C's 0.179 (pilot: 0.121 / 0.148 / 0.205), at citation coverages of 0.489, 0.526 and 0.519 respectively. The ordering is unchanged, so selectivity does not explain it. One caveat: the per-cell version of this check is weak—restricted to the seven cells where E2 cites at least as often as D, E2 has the lower MAR in four. The aggregate is the evidence; the per-cell story is a coin flip.
