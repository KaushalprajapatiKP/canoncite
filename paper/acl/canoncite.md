...
- **C3 — A characterised selection rule (secondary).** A joint discriminative exact-ID selector (E2) that significantly improves on the reranking baseline (95% CI excludes zero under two aggregations) at two LLM calls per item, and is directionally—but *not* significantly—better than the reproduced SOTA once abstention is removed from the denominator. Its advantage is capacity-gated: below a threshold between 8B and 14B the decomposed baseline wins, a reversal that survives end-to-end replication. We present E2 as a probe of *why* the residual errors are hard, not as a system we claim beats the state of the art (§Systems and Results).
...
Our margins are close enough to measurement noise that a point estimate cannot carry the claim, so we test them. We resample the 14 cross-lingual cells with replacement (B=10,000) and recompute each pairwise margin, reporting 95% intervals (Table “boot”). Two results follow, and they differ.
Against the reranking baseline C, **E2's advantage is significant under both aggregations**: -0.101 macro MAR (95% CI [-0.164, -0.046]) and -0.055 wrong-citations-per-item (CI [-0.093, -0.015]). Joint discriminative selection reliably improves on reranking alone.
Against the reproduced SOTA D, **it is not established**. The macro margin is significant (-0.070, CI [-0.159, -0.004]) but the coverage-corrected per-item margin is not (-0.024, CI [-0.061, +0.011]), and an item-level bootstrap agrees (CI [-0.041, +0.009]). Since the per-item metric is the one that answers the abstention objection, we take the conservative reading: **E2 is directionally better than D across every aggregation, reader and metric we measured, but this study is not powered to establish it.** We state that as the result rather than reporting the aggregation under which the gate passes and omitting the one under which it does not. Confirming or refuting it needs more cells—more corpora, or more languages per corpus—not more analysis of these.

| **Pair** | **Metric** | **Margin** | **95% CI** |
|---|---|---|---|
| E2 vs C | macro MAR | -0.101 | [-0.164,-0.046] |
| E2 vs C | wrong/item | -0.055 | [-0.093,-0.015] |
| E2 vs D | macro MAR | -0.070 | [-0.159,-0.004] |
| E2 vs D | wrong/item | -0.024 | [-0.061,+0.011] |
| D vs C | macro MAR | -0.030 | [-0.077,+0.019] |

*Paired bootstrap over cross-lingual cells (B=10,000), negative favours the first system. E2 improves significantly on the reranking baseline under both aggregations; its advantage over the SOTA is directional but not significant once abstention is removed from the denominator.*

**Why we report no per-corpus ordering.**  

Cell-level uncertainty is far from uniform, because MAR's denominator is the number of *citing* items, which ranges from 9 to 80 across our cells. Rāmāyaṇa Hindi has 9, giving its MAR a binomial standard error near 0.10, so an apparent 0.065 difference there is a single item. Two cells have no MAR at all: C cited nothing under Hindi queries for Bible and the Constitution (n_textciting=0), which leaves the rate undefined rather than indicating a missing run. Per-corpus values are in the released JSONL; we draw no conclusions from them.

**Aggregation, and why we report both.**  

MAR is defined over *citing* items, so a cell's value is a ratio whose denominator differs by system. Averaging those ratios across cells (macro) weights a 12-item Gurmukhi condition equally with an 80-item Bible one; pooling wrong citations over all citing items (micro) weights by volume. The choice is not innocuous here: the E2-versus-D cross-lingual margin is -0.054 macro but -0.011 micro. We report macro throughout, because the benchmark is designed so that each corpus~×~language condition is an experimental unit of equal interest, and pooling would let the two largest corpora determine a result about cross-lingual generality. But the micro margin sits inside our ±0.014 noise band, so a reader who prefers volume-weighting should treat the ordering as unresolved at the aggregate level rather than established. We state this rather than let the aggregation choice do silent work.
