...
**Realized so far.** The full ten-corpus grid reported in §5.4 has been run to completion with **Qwen2.5-14B-Instruct** as the primary reader (self-hosted, all systems A–D/E2). A second, independently-trained open multilingual reader, **Aya-Expanse-8B** (Cohere For AI), has been run to completion as a cross-reader robustness check on the winning configurations (C, E2, D — all 28 cells each; §5.4.7); GPT-4o-class and further diversity readers remain future work, pending API budget.
