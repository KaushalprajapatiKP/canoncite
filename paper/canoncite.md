...
> **Working draft.** This document fixes the problem statement, the benchmark design, the metric definitions, the experimental protocol, and the precise prior-art positioning. The **system ladder is now instantiated on the two cleanest corpora (Bhagavad Gītā, Yoga Sūtras) with a Qwen2.5-14B reader**: System A (all 28 cells, §5.4), and Systems **B (hybrid), C (reranking), E (verify+repair), and E2 (our discriminative exact-ID selector)** in §5.4.2–§5.4.5. The retrieval ladder A→B→C lifts cross-lingual F1 0.177→0.626→0.690, and **E2 roughly halves misattribution** at matched F1. The **all-10-corpus scale-up and System D (Self-RAG+CRAG) baseline are complete, and the pre-committed E2-vs-D gate PASSES**, E2 attains the lowest cross-lingual misattribution rate of any system across all ten corpora (0.387 vs D's 0.443, at ~2 LLM calls/item vs ~10), the **money table (Table 5.6)**. Cross-reader robustness is now established at **three capacity points**, Aya-Expanse-8B (28 cells), Qwen2.5-14B (28 cells), and gpt-oss-120B (pilot corpora), showing the gate is **reader-capacity-dependent** and that E2's margin **widens monotonically with reader scale** (§5.4.7, Tables 5.12–5.13). Remaining controls (Tables 5.3–5.5) are FORTHCOMING. **v0 status, read this first:** every empirical result reported here is measured and reproducible, but the benchmark items themselves are **model-drafted and not yet human-verified** (0 of 622 items reviewed). Human verification is instrumented and pending; inter-annotator agreement and judge calibration are therefore *not* reported. See the abstract's "Status of this release" and §7.
...
**Status of this release (v0).** CANONCITE v0 ships **model-drafted items** whose gold citations are sampled from, and automatically validated against, the closed corpus ID space. Human verification is fully instrumented, a deployed multilingual review interface and an agreement→gold adjudication pipeline (Krippendorff α-MASI / Cohen–Fleiss κ), but **has not yet been carried out**, so inter-annotator agreement and content-support judge calibration are reported as *pending* rather than measured (Tables 5.4–5.5). All system comparisons below are therefore **relative** comparisons over a common, automatically-validated label set: the caveat applies identically to every system, so the orderings we report are unaffected by it, while absolute values may shift once human-verified gold lands.
...
- **C1, Benchmark (headline).** CANONCITE, the first **multilingual, multi-tradition, multi-script** canonical-citation attribution + abstention benchmark: ten public-domain corpora, 188,557 citable units across five scripts, closed canonical-ID ground truth, a multilingual text triple, an ambiguity-graded five-type question taxonomy, near-miss-citation distractors, and dedicated abstention items. **Item provenance (v0):** questions are model-drafted, with gold citations *sampled from and automatically validated against* the closed corpus ID space, and Hindi/native question translations produced by IndicTrans2-1B; **as of this release none of the 622 items has been human-verified** (§3.5, §7). The **cross-lingual / multi-script attribution axis is the freshest contribution**, it is unoccupied across the entire attribution lineage. We lead with C1.

- **C2, Metric suite (integration + validation, not invention).** A unified, corpus-agnostic **exact-attribution** metric suite (§4). We explicitly credit Ovcharov (2026) and CAQA for the fixed-ID existence-check idea and ALCE for the citation precision/recall paradigm. Our contribution here is (i) *integrating* these into one script-agnostic harness that operates over normalized IDs, and (ii) *specifying and instrumenting* a validation protocol for the exact-ID signal (and a calibrated content-support check) against human judgment across religious and legal corpora, a cross-lingual generality claim no prior fixed-ID work makes. **In this v0 release, (i) is complete and (ii) is specified and instrumented but not yet executed:** no human verdicts have been collected, so Tables 5.4–5.5 are pending and **the validation half of C2 is not yet supported by evidence.** We state this rather than let the contribution read as established.

- **C3, Method (secondary, gated).** A **joint discriminative exact-ID selector** (System E2): rather than the crowded per-passage verify-and-repair loop (CiteFix, VeriCite, RARR, CG-DPO, Self-RAG/CRAG), which our own results show is *subsumed* once retrieval ranking is fixed (§5.4.4), E2 presents the reranked candidates jointly and forces a single exact-source choice or abstention, resolving the **near-miss** errors (adjacent same-theme verse) that a topical reranker and a binary verifier structurally cannot. It **roughly halves Misattribution Rate at matched F1** on the two pilot corpora (§5.4.5), and **passes its pre-committed gate at scale**: across all ten corpora it attains the **lowest cross-lingual Misattribution Rate of any system (0.387 vs. the reproduced Self-RAG/CRAG SOTA's 0.443)** at ~2 LLM calls/item versus ~10, and with *no* NLI model, verification is an exact lookup against the closed ID space (Table 5.6). A three-point cross-reader check spanning 8B–120B (§5.4.7) bounds the claim honestly *and* extends it: the advantage **requires a sufficiently capable reader**, under an 8B reader the decomposed SOTA is the more robust choice on the full grid, but above that threshold it **widens monotonically with reader scale** (MAR margin over the SOTA 0.013 → 0.027 → 0.081; misattribution cut over the reranking baseline −29% → −40% → −61%). C3 is thus a capacity-matched selection rule rather than an unconditional win, whose preferred regime is the frontier one.
...
| **Total** | 6 traditions | 5 scripts | closed per-corpus `U` | **188,557** | public-domain | n/a |
...
**Realized so far.** The full ten-corpus grid reported in §5.4 has been run to completion with **Qwen2.5-14B-Instruct** as the primary reader (self-hosted, all systems A–D/E2). A second, independently-trained open multilingual reader, **Aya-Expanse-8B** (Cohere For AI), has been run to completion as a cross-reader robustness check on the winning configurations (C, E2, D, all 28 cells each; §5.4.7). A third, **gpt-oss-120B** (OpenAI open weights, served via Cerebras), has been run on the same three systems over the two pilot corpora, giving three reader capacities spanning 8B–120B; its all-ten scale-up, and proprietary GPT-4o-class readers, remain future work pending compute and API budget.
...
#### 5.4.7 Cross-reader robustness across three capacity points, the gate is reader-dependent

To test whether the E2-vs-D result is an artifact of the primary reader, we re-ran the three head-to-head systems (C, E2, D) across the full grid with **Aya-Expanse-8B** (Cohere For AI), an independently trained, explicitly multilingual open reader roughly half Qwen2.5-14B's size, under an identical protocol (same items, retrieval, prompts, and k; only the reader swapped). All 28 cells of all three systems are complete; comparisons below are over the **18 cross-lingual cells where both E2 and D yield a defined MAR under both readers**, so the reader contrast is apples-to-apples. Numbers verbatim from `results/gpu_qwen14b/system{C,E2,D}_aya.jsonl`.

**Table 5.12, The E2-vs-D gate under two readers (matched 18 cross-lingual cells): Misattribution Rate and cells won.**

| Reader | E2 XL-MAR ↓ | D XL-MAR ↓ | E2 beats-or-ties D (cells) | Gate |
|---|---:|---:|---:|---|
| **Qwen2.5-14B** (primary) | **0.387** | 0.443 | 13 / 18 | **holds** |
| **Aya-Expanse-8B** (robustness) | 0.604 | **0.577** | 8 / 18 | **flips** |

**Finding, E2's advantage is contingent on reader capacity; the gate does not transfer to an 8B reader.** Under Aya-8B every system degrades (XL-F1: C 0.303, E2 0.331, D 0.369, all far below the Qwen grid), but D degrades *least* and overtakes E2 on the gated axis (XL-MAR 0.577 vs 0.604, winning 10 of 18 cells). The mechanism is visible in the intervention behavior: under the weaker reader, E2's single joint-selection call becomes erratic, repairs fire 499 times (vs 176 for D) and abstention rises to 26.4% of items (vs 16.7%), i.e., the one-call discriminative select **relies on the reader having the capacity to compare eight candidates side-by-side and hold the exact-source criterion**, whereas D decomposes verification into many *individually easy* per-passage judgments (relevant? supported?) that an 8B model can still execute. English-query cells are essentially tied (MAR 0.474 vs 0.481), so the flip is specifically cross-lingual, where the discrimination is hardest.

**A third capacity point at the top of the range.** To test the other direction, whether E2's advantage *widens* with reader capacity, as the mechanism above predicts, we ran the same three systems with **gpt-oss-120B** (OpenAI open weights, served via Cerebras) under the identical protocol. This reader is roughly an order of magnitude larger than Aya-8B and covers the two pilot corpora (Bhagavad Gītā, Yoga Sūtras); the all-ten scale-up at this capacity is left to future work, so the comparison below is stated on the **matched pilot cells** rather than the 28-cell grid. Numbers verbatim from `results/gpu_qwen14b/system{C,E2,D}_oss120b.jsonl`.

**Table 5.13, The E2-vs-D gate across three reader capacities (matched pilot cross-lingual cells: Gītā + Yoga Sūtras × hi/sa, 4 cells, n = 264 items).**

| Reader | C XL-MAR ↓ | E2 XL-MAR ↓ | D XL-MAR ↓ | E2 − D | E2 beats-or-ties D | E2 XL-F1 | D XL-F1 |
|---|---:|---:|---:|---:|---:|---:|---:|
| **Aya-Expanse-8B** | 0.441 | **0.315** | 0.328 | −0.013 | 1 / 4 | 0.529 | **0.559** |
| **Qwen2.5-14B** | 0.253 | **0.152** | 0.179 | −0.027 | 3 / 4 | 0.685 | **0.687** |
| **gpt-oss-120B** | 0.313 | **0.121** | 0.202 | **−0.081** | **4 / 4** | 0.681 | **0.699** |

**Finding, E2's misattribution advantage grows monotonically with reader capacity.** On the matched pilot cells the margin over D widens from 0.013 (8B) to 0.027 (14B) to **0.081 (120B)**, and E2's cut in misattribution relative to the reranking baseline C strengthens in step: **−29% (8B), −40% (14B), −61% (120B)**. At 120B, E2 attains the lowest cross-lingual MAR we measure for any system–reader pair (0.121) and beats D in **all four** cross-lingual cells, including the two Sanskrit conditions where near-miss discrimination is hardest (Gītā-sa 0.239 → 0.177; Yoga-sa 0.175 → 0.065). The English cells move the same way (E2 0.062 vs D 0.109), reversing the near-tie seen at 8B. The F1 ordering is unchanged at every capacity, D retains a slim edge (0.699 vs 0.681 at 120B), which is consistent with §5.4.5: F1 is not what a verification layer optimises, and the gate is defined on misattribution.

**An important scope limit on this comparison.** The 120B column is measured on 2 corpora, not 10, so it is *not* interchangeable with the 28-cell result in Table 5.12. Note also that on this pilot subset the gate holds for **all three** readers, including Aya-8B, which flips only on the full 28-cell grid. The pilot cells therefore do not by themselves discriminate by capacity; what Table 5.13 establishes is that at the top of the range the margin is the **widest observed**, in the direction the capacity mechanism predicts. Locating the crossover exactly, and confirming the 120B margin survives the seven harder corpora (notably the long-prose and low-resource-script conditions where every system degrades), requires the all-ten grid at this capacity and remains open.

**Interpretation for C3.** The method claim is therefore sharpened, not withdrawn: **with a sufficiently capable reader, joint discriminative exact-ID selection attains the lowest misattribution of any system at ~1/5 the SOTA's inference cost (Table 5.6), and its advantage widens as reader capacity grows (Table 5.13); below that capacity, the SOTA's decomposed per-passage reflection is the more robust choice, its ~10 calls/item buy back, through decomposition, what the weak reader cannot do in one joint judgment.** This yields an actionable selection rule (match the verification structure to the reader's capacity) and locates the capability threshold for this task family between **8B and 14B**, with the margin still increasing an order of magnitude above it. Because frontier-scale readers are the deployment target for citation-critical systems, the regime in which E2 is preferred is the one that matters in practice, and it is the cheaper one.
...
- **No human verification has been performed yet (the primary limitation of v0).** All 622 items are model-drafted and machine-translated; their gold citations are sampled from and automatically validated against the closed corpus ID space, but **no item has been checked by a human annotator, and no inter-annotator agreement has been measured.** The review interface and the agreement→gold adjudication pipeline are built and deployed, so this is a matter of annotation effort rather than missing method, but until it is done, item-level label correctness (does this *question* genuinely target this *verse*?) and translation adequacy for Hindi/native queries are unverified. Readers should treat absolute metric values as provisional. The system *orderings* are more robust, since every system is scored against the same labels; a label-noise floor affects them all equally.
...
The empirical study is complete on its primary reader (Qwen2.5-14B): the full system ladder, naive RAG, hybrid, reranking, a reproduced Self-RAG+CRAG SOTA, and our joint discriminative exact-ID selector (E2), was run across all ten corpora and all query-language conditions, with **0 timeouts and 0 OOM**. The pre-committed decision gate **passes**: E2 attains the lowest cross-lingual Misattribution Rate of any system (0.387, ~13% below the SOTA's 0.443) at roughly a fifth of the inference cost, confirming that the cross-lingual collapse is fundamentally a retrieval-ranking failure and that the residual near-miss misattribution is best resolved by joint discrimination rather than per-passage reflection or NLI verification. A three-point cross-reader study (Aya-Expanse-8B, Qwen2.5-14B, gpt-oss-120B; §5.4.7) adds the study's most instructive nuance: the gate is **reader-capacity-dependent**, under an 8B reader the SOTA's decomposed reflection overtakes the joint select on the full grid, while above that threshold E2's advantage **widens monotonically with scale**, reaching a 0.081 MAR margin and a 61% cut in misattribution over the reranking baseline at 120B. The method contribution therefore lands as a **capacity-matched selection rule** (one cheap joint call when the reader can carry it; decomposed per-passage verification when it cannot), with the crossover located between 8B and 14B and the preferred regime being both the frontier one and the cheaper one. **Remaining before submission to a peer-reviewed venue:** first and foremost **human verification of the benchmark items**, this release's primary limitation (§7) and the precondition for the Tier-A inter-annotator agreement (Table 5.4) and the content-support judge calibration (Table 5.5); then the closed-book base-competence control (Table 5.3) and the all-ten scale-up of the 120B reader column. We release v0 now, with these gaps stated plainly, so the benchmark, the harness, and the system ladder are available for scrutiny and reuse while that work proceeds.
